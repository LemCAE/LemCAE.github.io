...
from tkinter import StringVar,Menu,messagebox,Toplevel
...
def show(show_name):
    json_data["hide_values"][value_name.index(show_name.rstrip(show_name[-1]))][show_name] = ""
...
def set():
    setting_window = Toplevel(root_window)
    setting_window.title("设置")
    setting_window.geometry("300x400")
    setting_window.resizable(False, False)
    
    stored_autohide_value = json_data["settings"][0]["auto_hide"]
    if stored_autohide_value == "close":
        defult_checkbutton_ath = "0"
    else:
        defult_checkbutton_ath = "1"
    checkVar1 = StringVar(value=defult_checkbutton_ath)
    auto_hide = Checkbutton(setting_window, text="自动隐藏空项(暂不可用)", variable=checkVar1)
    auto_hide.place(x=10, y=10)
    
    def save_exit():
        if checkVar1 == "1":
            json_data["settings"][0]["auto_hide"] = "on"
        else:
            json_data["settings"][0]["auto_hide"] = "off"
        with open(resource_path('homeworkboard_setting.json'), 'w', encoding="utf-8") as g:
            json.dump(json_data, g,indent=4, ensure_ascii=False)
        setting_window.destroy()
           

    close_setting_unsave = Button(setting_window, text="取消", command=setting_window.destroy)
    close_setting_unsave.pack()
    close_setting_unsave.place(x = 120, y = 370)
    close_setting_save = Button(setting_window, text="确认", command=save_exit)
    close_setting_save.pack()
    close_setting_save.place(x = 210, y = 370)
